...
thresholdupper "" r (0, 0.05)
thresholdlower "" r (0, 0.05)
'
...
for (acao in acoes) {
  ...
  sink(file = paste("LogsMomentum/parametros-Momentum-configs-", acao, ".txt"))
  ...
  adhoc[i] <- test(data.frame(price='close', period=3, ma='SMA', thresholdupper = 0.01, thresholdlower=0.01 , symbol = acao, start = dt_start, end = dt_end))
...
#pdf(file='param-Momentum.pdf', width=4, height=4)
...
#        main='Momentum: Sintonia')
#dev.off()

#sink(file = "parametros-Momentum.txt")
