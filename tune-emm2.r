...
target.runner <- function(experiment, scenario) {
  ...
  obj <- heuristicaEMM2(
    ...
    as.integer(configuration[['x3']]),
    as.double(configuration[['x4']]),
    as.double(configuration[['x5']]),
    as.double(configuration[['x6']]),
    as.double(configuration[['x7']]),
    as.double(configuration[['x8']]),
    as.double(configuration[['x9']]),
    ticker = instance,
...
for (acao in acoes) {
  ...
  sink(file = paste("LogsEMM2/parametros-emm2-configs-", acao, ".txt"))
  ...
  adhoc[i] <- test(data.frame(x0='close', x1=3, x2=15, x3 = 30, x4=0.1,x5=0.1, x6=0.1,x7=0.1, x8=0.1, x9=0.1, ticker = acao, inicio_periodo = dt_start, fim_periodo = dt_end))
...
#pdf(file='param-EMM2.pdf', width=4, height=4)
