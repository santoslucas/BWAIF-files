...
class avalia_parametros_emm1(ElementwiseProblem):
    def __init__(self, ticker: str, dt_start: str, dt_end: str, **kwargs):
        ...
        vars = {
            ...
            'x3': Integer(bounds=[10, 12]),
            'x4': Real(bounds=[0.01, 0.15]),
            'x5': Real(bounds=[0.01, 0.15]),
            'x6': Real(bounds=[0.01, 0.15]),
            'x7': Real(bounds=[0.01, 0.15]),
            'x8': Real(bounds=[0.01, 0.15]),
            'x9': Real(bounds=[0.01, 0.15])
        }
...
np.savetxt('objvls-emm2-ga-tuned.txt', f)