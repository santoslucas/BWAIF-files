...
class avalia_parametros_reversion(ElementwiseProblem):
    def __init__(self, symbol: str, dt_start: str, dt_end: str, **kwargs):
        ...
        vars = {
            ...
            'period': Integer(bounds=[2, 60]),
            'ma': Choice(options=['SMA', 'EMA', 'DEMA', 'TEMA', 'TRIMA', 'KAMA']),
            'thresholdupper': Real(bounds=[0, 5]),
            'thresholdlower': Real(bounds=[0, 5])
        }
...
for ticker in tickers:
    problem = avalia_parametros_reversion(ticker, dt_start, dt_end)
...
np.savetxt('objvls-reversion-ga-tuned.txt', f)